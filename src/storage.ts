...
const defaults: Profile = { name: 'Misafir', coins: 25000, games: 0, wins: 0, lastBonus: '', vibration: true, quick: false, theme: 'green' };
...
const toRow = (profile: Profile, userId: string) => ({ id: userId, name: profile.name, vibration: profile.vibration, quick: profile.quick, theme: profile.theme });
    ...
    if (userId && supabase) {
      ...
      AsyncStorage.getItem('keyif-profile-v1').then(raw => {
        ...
        try {
          ...
          if (typeof saved.name === 'string' && Number.isFinite(saved.coins) && Number.isFinite(saved.games) && Number.isFinite(saved.wins)) setProfile({ ...defaults, ...saved, coins: saved.coins === 12500 && saved.games === 0 && saved.wins === 0 ? 25000 : saved.coins });
...
export async function claimDailyBonus() {
  ...
  return { coins: typeof data === 'number' ? data : null, error: error?.message ?? null };
}
