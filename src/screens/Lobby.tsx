...
import { View, Text, Pressable, ScrollView, StyleSheet, useWindowDimensions, Switch, TextInput } from 'react-native';
...
export type Room = { title: string; subtitle: string; mode: Mode; theme: Profile['theme']; tag: string; icon: typeof Coffee; reward: number };
export const ROOMS: Room[] = [
  { title: 'Klasik masa', subtitle: 'Bildiğin oyun, sevdiğin keyif.', mode: 'classic', theme: 'green', tag: 'EN SEVİLEN', icon: Coffee, reward: 500 },
  { title: 'Çifte gidelim', subtitle: 'Biraz şans, iki kat strateji.', mode: 'pairs', theme: 'plum', tag: 'ÇİFT AÇMALI', icon: Layers3, reward: 750 },
  { title: 'Gece masası', subtitle: 'Sakin bir masa, son bir el.', mode: 'classic', theme: 'blue', tag: 'GECE MODU', icon: Moon, reward: 500 },
];
...
function RoomCard({ room, onPress, mobile }: { room: Room; onPress: () => void; mobile: boolean }) {
  const Icon = room.icon;
  const tint = room.theme === 'plum' ? '#b59aa7' : room.theme === 'blue' ? '#9eafc5' : '#b4c79b';
  return <Pressable accessibilityRole="button" accessibilityLabel={`${room.title}, botlara karşı oyna`} onPress={onPress} style={({ pressed, hovered }: { pressed: boolean; hovered?: boolean }) => [s.roomCard, mobile && { width: '100%', flex: undefined }, { borderColor: hovered ? tint : C.border, opacity: pressed ? .85 : 1 }]}>
    ...
    <View style={s.roomFooter}><View style={s.inline}><Users size={13} color={C.muted} /><Text style={s.small}>3 bot + sen</Text></View><View style={s.inline}><Coin size={16} /><Text style={[s.small, { color: C.gold }]}>+{room.reward} ödül</Text></View></View>
  </Pressable>;
}

export default function Lobby({ page, profile, update, onStart, navigate, onCustom, onBonus, onSignOut }: Props) {
  const { width } = useWindowDimensions();
  const mobile = width < 700, narrow = width < 1180;
  ...
  return <ScrollView style={{ flex: 1 }} contentContainerStyle={[s.content, mobile && { padding: 22, paddingBottom: 30 }]} showsVerticalScrollIndicator={false}>
    <View style={[s.pageHeading, mobile && { marginBottom: 24 }]}><View><Label color={C.green}>{page === 'lobby' ? 'İYİ Kİ GELDİN' : 'KEYİF 101'}</Label><Text accessibilityRole="header" style={[s.heading, mobile && { fontSize: 38 }]}>{titles[page][0]}</Text><Text style={s.subtitle}>{titles[page][1]}</Text></View>{!mobile && <View style={s.modeBadge}><View style={s.statusDot} /><Text style={s.small}>Botlarla, kendi temponda</Text></View>}</View>

    {page === 'lobby' && <>
      <View style={[s.heroRow, narrow && { flexDirection: 'column' }]}>
        <LinearGradient colors={['#345a43', '#254b39', '#204534']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={[s.hero, mobile && { height: 490, padding: 26 }]}>
          <Grain />
          ...
          <View style={[s.heroArt, mobile && { right: -21, top: 160 }]}><TableArt scale={mobile ? .83 : width < 1450 ? .88 : 1} /></View>
          ...
          <Text style={[s.heroTitle, mobile && { fontSize: 38, marginTop: 18 }]}>Hoş sohbet,{ '\n' }iyi oyun.</Text>
          <Text style={[s.heroDescription, mobile && { maxWidth: 235 }]}>Bir tutam şans, biraz strateji.{ '\n' }101’in en keyifli hâline hoş geldin.</Text>
          <View style={mobile ? { marginTop: 'auto', zIndex: 2 } : { marginTop: 25, alignSelf: 'flex-start', zIndex: 2 }}><Button onPress={() => onStart(ROOMS[0])} icon={<ArrowRight size={19} color={C.ink} />} style={mobile ? { width: '100%' } : { minWidth: 194 }} testID="quick-play">Hemen oyna</Button></View>
          {!mobile && <View style={[s.inline, { marginTop: 18, gap: 9 }]}><ShieldCheck size={13} color="#a9bca1" /><Text style={{ fontFamily: F.medium, color: '#bbcbb2', fontSize: 10 }}>Ücretsiz masa · Beklemeden oyna</Text></View>}
        </LinearGradient>
        <LinearGradient colors={['#eee1c4', '#d9c297']} style={[s.reward, narrow && { width: '100%', minHeight: undefined, flexDirection: 'row', alignItems: 'center', padding: 22, gap: 18 }, mobile && { flexWrap: 'wrap' }]}>
          <View style={narrow ? { flex: 1 } : {}}><View style={[s.inline, { gap: 6 }]}><Gift size={14} color="#78603e" /><Label color="#78603e" style={{ fontSize: 9 }}>KÜÇÜK BİR HOŞ GELDİN</Label></View>
            <Text style={[s.rewardTitle, narrow && { fontSize: 24, marginTop: 9 }]}>Günün{narrow ? ' ' : '\n'}güzelliği.</Text><Text style={s.rewardDesc}>Yeni bir gün, yeni bir şans.</Text></View>
          {!narrow && <View style={{ height: 90, alignItems: 'center', justifyContent: 'center', marginVertical: 3 }}><View style={{ transform: [{ rotate: '-12deg' }] }}><Coin size={60} /></View><View style={{ position: 'absolute', right: 28, top: 34 }}><Coin size={37} /></View><Sparkles size={18} color="#a8874d" style={{ position: 'absolute', left: 25, top: 12 }} /></View>}
          <View style={narrow ? { alignItems: 'flex-end' } : {}}><Text style={[s.rewardValue, narrow && { fontSize: 25 }]}>+750 <Text style={{ fontSize: 13, fontFamily: F.medium }}>çip</Text></Text><Pressable accessibilityRole="button" disabled={claimed} onPress={onBonus} style={({ pressed }) => [s.rewardButton, { opacity: pressed ? .7 : 1 }, narrow && { paddingHorizontal: 18, marginTop: 7 }]}><Text style={{ color: '#fff4db', fontFamily: F.bold, fontSize: 12 }}>{claimed ? 'Bugünkü hediyen alındı' : 'Hediyeni al'}</Text>{claimed ? <Check size={15} color="#fff4db" /> : <ArrowRight size={15} color="#fff4db" />}</Pressable>{!narrow && <Text style={{ textAlign: 'center', color: '#755f40', fontSize: 9, fontFamily: F.medium, marginTop: 9 }}>{claimed ? 'Yarın yeniden görüşürüz.' : 'Her gün yeni bir hediye seni bekler.'}</Text>}</View>
        </LinearGradient>
      </View>
      <View style={{ marginTop: 32 }}><SectionTitle title="Nasıl bir oyun olsun?" action="Tüm masalar" onPress={() => navigate('tables')} /><View style={[s.rooms, mobile && { flexDirection: 'column' }]}>{ROOMS.map(room => <RoomCard key={room.title} room={room} mobile={mobile} onPress={() => onStart(room)} />)}</View></View>
      <View style={[s.bottomBanner, mobile && { padding: 20 }]}><View style={s.bannerIcon}><Leaf size={25} color={C.green} strokeWidth={1.4} /></View><View style={{ flex: 1 }}><Text style={{ fontFamily: F.bold, fontSize: 14, color: C.text }}>Aceleye gerek yok.</Text><Text style={{ fontFamily: F.regular, fontSize: 12, color: C.muted, marginTop: 5, lineHeight: 19 }}>Reklamsız, beklemesiz. Sadece sen, taşların ve güzel bir oyun.</Text></View>{!mobile && <View style={{ width: 1, height: 34, backgroundColor: C.border, marginHorizontal: 20 }} />}{!mobile && <Pressable onPress={() => navigate('rules')} accessibilityRole="button" style={[s.inline, { minHeight: 44 }]}><Text style={{ color: C.green, fontFamily: F.bold, fontSize: 11 }}>101’e yeni misin?</Text><ArrowUpRight size={16} color={C.green} /></Pressable>}</View>
    </>}

    {page === 'tables' && <>
      <View style={[s.filterRow, mobile && { flexWrap: 'wrap' }]}>{([['all', 'Tüm masalar'], ['classic', 'Klasik 101'], ['pairs', 'Çift açmalı']] as const).map(([value, label]) => <Pressable accessibilityRole="button" accessibilityState={{ selected: filter === value }} key={value} onPress={() => setFilter(value)} style={[s.filter, filter === value && { backgroundColor: C.green, borderColor: C.green }]}><Text style={{ fontFamily: F.bold, color: filter === value ? C.ink : C.muted, fontSize: 12 }}>{label}</Text></Pressable>)}</View>
      <View style={[s.rooms, mobile && { flexDirection: 'column' }]}>{ROOMS.filter(r => filter === 'all' || r.mode === filter).map(room => <RoomCard key={room.title} room={room} mobile={mobile} onPress={() => onStart(room)} />)}</View>
    ...
    {page === 'stats' && <>
      ...
      <View style={[s.rooms, mobile && { flexDirection: 'column' }]}>{[[Trophy, 'Kazanılan el', profile.wins], [Layers3, 'Oynanan el', profile.games], [Zap, 'Kazanma oranı', `%${profile.games ? Math.round(profile.wins / profile.games * 100) : 0}`]].map(([Icon, label, value], i) => { const I = Icon as typeof Trophy; return <View key={i} style={[s.infoPanel, { flex: 1 }]}><I color={C.gold} size={22} /><Text style={{ fontFamily: F.serif, fontSize: 46, color: C.text, marginVertical: 12 }}>{String(value)}</Text><Text style={s.subtitle}>{String(label)}</Text></View>; })}</View>
    ...
    {page === 'settings' && <View style={[s.infoPanel, { maxWidth: 740 }]}><Label color={C.gold}>MASADA SANA NE DİYELİM?</Label><TextInput accessibilityLabel="Oyuncu adı" value={name} onChangeText={v => { setName(v); setSaved(false); }} maxLength={18} placeholder="Oyuncu adın" placeholderTextColor={C.muted} style={s.nameInput} /><Button onPress={() => { update({ name: name.trim() || 'Misafir' }); setSaved(true); }} style={{ alignSelf: 'flex-start', marginBottom: 22 }} compact icon={saved ? <Check color={C.ink} size={16} /> : undefined}>{saved ? 'Kaydedildi' : 'Adını kaydet'}</Button>
      {([{ label: 'Dokunsal geri bildirim', desc: 'Taşlara dokunduğunda hafif titreşim. Desteklenen telefonlarda çalışır.', key: 'vibration' }, { label: 'Hızlı botlar', desc: 'Bilgisayar oyuncuları hamlelerini daha hızlı yapar.', key: 'quick' }] as const).map(item => <View key={item.key} style={s.settingRow}><View style={{ flex: 1 }}><Text style={s.settingTitle}>{item.label}</Text><Text style={[s.subtitle, { lineHeight: 20, marginTop: 5 }]}>{item.desc}</Text></View><Switch accessibilityLabel={item.label} value={profile[item.key]} onValueChange={value => update({ [item.key]: value })} trackColor={{ false: '#466157', true: '#8faa7c' }} thumbColor={C.text} /></View>)}
