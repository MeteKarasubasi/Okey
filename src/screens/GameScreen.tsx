...
import { ChevronDown, Clock3, Gift, MessageCircle, RotateCcw, ShoppingBag, Trophy, ArrowDown, Users } from 'lucide-react-native';
...
export default function GameScreen({ game, setGame, profile, userId, remoteGameId, room, round, roundCount, matchScores, onUpdateProfile, onExit, onHelp, onReplay, paused, playerSeat = 0, onRemoteAction, onlineRoomId, onlineStarted = true, onlinePlayers = [], onlineCountdownEndsAt = null, onlineStartingUntil = null }: Props) {
  ...
  return <View testID="game-viewport" style={s.viewport} onLayout={e => setViewport({ width: e.nativeEvent.layout.width, height: e.nativeEvent.layout.height })}>
    <View testID="game-stage" style={[s.stage, { left: (viewport.width - W) / 2, top: (viewport.height - H) / 2, transform: [{ scale }] }]}>
      ...
      {onlineRoomId && !onlineStarted && (onlineCountdownEndsAt || onlineStartingUntil) && <View style={s.startOverlay} pointerEvents="auto"><View style={s.startModal}><View style={s.startIcon}><Clock3 size={29} color="#f4da7c" /></View><Text style={s.startEyebrow}>MASA HAZIR</Text><Text style={s.startTitle}>{startMessageVisible ? 'Oyun başlıyor' : 'Oyun Başlıyor'}</Text>{!startMessageVisible && <Text style={s.startNumber}>{startCountdown}</Text>}<View style={s.startPlayers}><Users size={16} color="#b6d8d0" /><Text style={s.startPlayersText}>4 gerçek oyuncu hazır</Text></View></View></View>}
      ...
      {game.ended && <View style={s.resultOverlay}><View style={s.result}><Trophy size={45} color={C.gold} /><Text style={s.resultTitle}>{round >= roundCount ? 'Maç tamamlandı.' : `${round}. el tamamlandı.`}</Text><Text style={s.resultCopy}>{game.winner === 0 ? `Bu elde +${room.reward} çip kazandın.` : game.winner === null ? 'Taşlar bitti, el berabere.' : `${NAMES[game.winner]} bu eli kazandı.`}</Text><Text style={s.matchScoreText}>TOPLAM SKOR · Sen {matchScores[0] ?? 0}</Text><Button onPress={onReplay} icon={<RotateCcw size={18} />}>{round >= roundCount ? 'Lobiye dön' : 'Sonraki el'}</Button><Button onPress={onExit} secondary>Oyundan çık</Button></View></View>}
    </View>
  </View>;
}
